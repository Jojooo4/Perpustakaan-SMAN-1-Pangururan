...
<div class="row mb-3">
    ...
    <div class="col-md-3">
        <a href="{{ route('petugas.denda.export', request()->query()) }}" class="btn btn-success w-100 mb-2">
            <i class="fas fa-file-excel me-2"></i>Export Excel
        </a>
        <a href="{{ route('petugas.denda.export-pdf', request()->query()) }}" class="btn btn-danger w-100">
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if(!($d->denda_lunas ?? false))
                            <button class="btn btn-success btn-sm" onclick="markPaidPetugas({{ $d->id_peminjaman }})">
...
document.getElementById('paymentModal').addEventListener('hidden.bs.modal', function () {
    if(shouldSubmit && selectedDendaId) {
        ...
        form.method = 'POST';
        form.action = `{{ route('petugas.denda.mark-paid', ['id' => 'ID_PLACEHOLDER']) }}`.replace('ID_PLACEHOLDER', selectedDendaId);
